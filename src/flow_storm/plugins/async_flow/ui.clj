  ...
  (try
    (let [graph-box (ui/v-box :childs [])
          ...
          *connections (atom nil)
                            ...
                            :on-click (fn []
                                        ...
                                        (let [flow-id @*graph-flow-id
                                              ...
                                              graph-pane (create-graph-pane {:on-edge-click
                                                                            ...
                                                                            conns)]
                                          (reset! *control-ch->pid control-ch->pid)
                                          ...
                                          (tasks/submit-task runtime-api/call-by-fn-key
                                                             [:plugins.async-flow/extract-messages-task
                                                              [@*graph-flow-id @*connections @*control-ch->pid]]
          ...
          toolbar-pane (ui/h-box :childs [(ui/label :text "Graph flow-id :") graph-flow-cmb reload-graph-btn loaded-msgs-lbl]
                                 :spacing 5
                                 :align :center-left
                                 :paddings [0 10 10 0])
...
(defn- on-focus [{:keys [graph-flow-cmb selected-graph-flow-id-ref]}]
